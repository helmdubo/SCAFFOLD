...
- **DD-33 � ScaffoldGraph is derived from endpoint relations and LoopCorners.**
  ...
  from `PatchChainEndpointRelation` and `LoopCorner`, not from raw Chain or
  world axes. ScaffoldEdge must be a graph-level view of final PatchChain, not
  a competing PatchChain identity. ScaffoldGraph must not be introduced before
