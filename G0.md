...
`ChainDirectionalRunUse` endpoint samples at the same Vertex. They must not use
U/V labels, H/V labels, WORLD_UP, WorldOrientation labels or runtime solve
output.

## DD-32 — Surface normals for ChainUse are derived facts, not Layer 1 topology

Layer 1 `ChainUse` must not store face normals or averaged normals. Owner
normals for junction relations are derived from Layer 2 geometry or Layer 3
evidence.
