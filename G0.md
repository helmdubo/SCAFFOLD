...
Future ScaffoldJunction:
  likely 2 seam endpoint groups after graph-level classification
...
not from raw Chain or world axes. `ScaffoldEdge` must be a graph-level view of
final PatchChain, not a competing PatchChain identity. ScaffoldGraph must not be
