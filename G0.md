...
```text
ScaffoldJunction
  future graph-level classification overlay on existing ScaffoldNode records,
  not a separate graph node identity.

Future ScaffoldJunctionKind vocabulary:
  SELF_SEAM, CROSS_PATCH, BRANCH, TERMINUS, AMBIGUOUS, DEGRADED.

SELF_SEAM:
  future ScaffoldJunctionKind for a ScaffoldNode where two incident final
  PatchChains share the same ChainId and same PatchId, representing the
  supported SEAM_SELF case.
...
ScaffoldJunction only when a future graph-level classifier assigns a
...
Planning-only ScaffoldJunctionKind vocabulary includes:
...
Ordinary ScaffoldNodes remain unclassified until the future ScaffoldJunction
classifier exists.
...
does not build ScaffoldTraces, ScaffoldCircuits or ScaffoldRails. Future
