...
Rules:
- Shared Chain / PatchChain tests for explicit Patch boundaries only.
...
def test_seamed_l_shape_shared_chain_has_two_opposite_uses() -> None:
    ...
    shared_chains = [
        ...
        if len(patch_chains_for_chain(model, chain.id)) == 2
    ]

    assert len(shared_chains) == 1

    uses = patch_chains_for_chain(model, shared_chains[0].id)
